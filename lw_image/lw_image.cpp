...
namespace lw {
	...
	Color& Image::pixel(size_t x, size_t y) {
		Assertion(0 <= x && x < _width && 0 <= y && y < _height,
			"Pixel index out of bounds");
		return _pixels[y * _width + x];
	}
	...
	void Image::loadBmp(const std::string& filename) {
		...
		const size_t lineSize = (sizeof(RGBTriple)* _width + 3) / 4 * 4;
	...
	void Image::saveBmp(const std::string& filename) const {
		const size_t lineSize = (sizeof(RGBTriple)* _width + 3) / 4 * 4;
		const size_t totalSize = lineSize * _height;
		const size_t offBits = sizeof(BitmapFileHeader) + sizeof(BitmapCoreHeader);
	...
	void Image::loadHdr(const std::string& filename) {
		...
		for (size_t y = 0; y < height; y++) {
			for (size_t x = 0; x < width; x++) {
				...
				_pixels[y * width + x] = lw::Color(r, g, b);
			}
		}
	...
	void Image::saveHdr(const std::string& filename) const {
		...
		for (size_t i = 0; i < _height; i++) {
			...
			for (int c = 0; c < 4; c++) {
				for (size_t cursor = 0; cursor < _width;) {
					const size_t cursor_move = std::min((size_t)127, _width - cursor);
					pixbuf.push_back(cursor_move);
					for (size_t j = cursor; j < cursor + cursor_move; j++) {
